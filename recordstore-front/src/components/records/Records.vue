  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <p class="pt-4">Don't see an artist? <router-link to="/artists" class="link-blue">Create one</router-link></p>
      </div>

      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-500 hover:bg-green-500 block w-full py-4 text-white items-center justify-center">
    </form>

    <hr class="border border-blue-light my-6" />
      ...
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-blue-light mt-4">
